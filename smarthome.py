...
def send_state_change(patient_id, device_id, new_state):
        ...
        if(res.status_code==200):
                ...
                raise Exception("An error occurred while setting state of device {}. HTTP Status: {}: {}".format(devices[device_id], res.status_code, res.text))
...
sc_url = "http://smartcare.mi.hdm-stuttgart.de/api/public/sleepcycle.json"
                ...
                if(last_sc['dateto']):
                        ...
                        res = requests.put(sc_url+"/{}".format(last_sc['id']), data={'dateTo': datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        if(res.status_code==200):
